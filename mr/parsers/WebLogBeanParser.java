...
public class WebLogBeanParser {
    ...
    public static WebLogBean parse(String line) {
        if (StringUtils.isEmpty(line)) {//StringUtils.isempty判断内容是否为空
            return null;
        }

        WebLogBean bean = new WebLogBean();//new实例 后面需要用到

        String[] fields = line.split(" ");//按照空格切分
    ...
    public static void filtStaticResource(WebLogBean bean, Collection<String> pages) {
        if (!pages.contains(bean.getRequest())) {//如果请求没有通过则脏数据
            bean.setValid(false);
        }
    }
